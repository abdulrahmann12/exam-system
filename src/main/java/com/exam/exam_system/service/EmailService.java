...
public class EmailService {
	...
	public void sendWelcomeEmail(UserRegisteredEvent user) {
		sendEmail(user.getEmail(), "Welcome to Exam System!", "emails/welcome",
				new ContextBuilder().add("username", user.getName()).build());
	}
	...
	public void sendCode(EmailChangeEvent user, String subject) {
		sendEmail(user.getNewEmail(), subject, "emails/send-code", // template path inside /resources/templates
				new ContextBuilder().add("name", user.getUsername()).add("code", user.getCode()).build());
	}
